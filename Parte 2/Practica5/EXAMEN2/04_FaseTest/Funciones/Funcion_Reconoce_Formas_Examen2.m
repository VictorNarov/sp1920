function Funcion_Reconoce_Formas_Examen2(Nombre)
...
load('02_DisegnoClasificador_Knn/DatosGenerados/datos_Knn_AD');
XTrainKnn = XTrain;
YTrainKnn = YTrain;
...
if Distancia == "Euclidea"
    load('03_DisegnoClasificador_Distancia/DatosGenerados/MDE_CE');
else
    load('03_DisegnoClasificador_Distancia/DatosGenerados/MDM_CE');
end
XTrainDistancia = XoIRed;
YTrainDistancia = YoIRed;
nombresProblemaDistancia = nombresProblemaOIRed;
espacioCcasDistancia = espacioCcas;


% Preparamos los conjuntos de datos reducidos
XTestRedKnn = XTest(:, espacioCcasKnn);
XTestRedDistancia = XTest(:, espacioCcasDistancia);
...
for i=1:numMuestras %POR CADA OBJETO DE LA IMAGEN    
    ...
    elseif numEuler == 0 % 1 hueco: A vs D KNN
       
        clase_objeto = nombresProblemaKnn.clases{YTestKnn(i)};      

        % Representacion
        axes(ax1);title(['numEuler = 0 -> KNN: Objeto pertenece clase:' clase_objeto]);
    ...
    elseif numEuler == 1 % sin huecos: C vs E Distancia
       
        x1 = XTestRedDistancia(i,1);
        x2 = XTestRedDistancia(i,2);
        x3 = XTestRedDistancia(i,3);

        d12_evaluada = eval(d12);

        % Representacion
        subplot(1,2,2),funcion_representa_muestras_clasificacion_binaria_con_frontera(XTrainDistancia, YTrainDistancia, coeficientes_d12, nombresProblemaDistancia);
        hold on, plot3(x1,x2,x3, 'dk');
        title(['x1= ' num2str(x1) 'x2= ' num2str(x2) 'x3= ' num2str(x3)]);
      ...
      if d12_evaluada > 0
          ...
          axes(ax1),title(['numEuler = 1 -> Distancia: Objeto pertenece clase:' clase_objeto]);
      else
